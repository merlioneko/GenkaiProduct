...
2. **機能**: `novel/schema.py` に Concept / Plot / Novel の pydantic モデルを定義する(SPEC.md D30〜D36)。変換はフェーズ関数に置き、モデルは gateway・prompts を import しない。Stage 抽象は作らない。旧 `novel/plot.py`・`novel/novel.py` は削除する。
...
- Plot機械検査が、意図的に仕込んだ未使用キャラ・未定義の場所・「または」・出来事の順序違反を検出する。
- U9(Concept スキーマの拡張要否)を M2 着手前に一度判断し、結果を SPEC.md に記録している。
