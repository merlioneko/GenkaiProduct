"""バックエンド設定(.env/backends.json)の読み込み(D25, D26)。秘密の値は表示しない。"""
...
class PhaseConfig(BaseModel):
    ...
    # None = 思考関連のパラメータを付けず、サーバ既定値に任せる(D26)
...
def load_backend(path: Path | str = DEFAULT_CONFIG, backend: str = "local") -> BackendConfig:
    """選んだバックエンドだけ検証する。local 実行に remote の鍵は要らない。"""
    try:
...
def load_thinking_options(path: Path | str | None) -> dict[str, Any]:
    """モデルごとの思考オン/オフ用リクエスト本文(U2 検証用)。未指定なら空。"""
    if path is None:
        return {}
    try:
        data = json.loads(Path(path).read_text(encoding="utf-8"))
...
def thinking_body(options: dict, phase: PhaseConfig) -> dict:
    """フェーズの thinking 設定に対応する extra_body を返す。

    thinking が None なら何も付けない。true/false なら、そのモデルの on/off 本文が
    options に無いかぎりエラーにする(サーバ既定値をオン/オフが効いたとみなさない)。
    """
    if phase.thinking is None:
        return {}
    ...
    if not isinstance(body, dict) or not body:
        raise ConfigError(
            ...
            "or set thinking to null to use the server default."
        )
