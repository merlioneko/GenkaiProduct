"""オフラインテスト。実際の OpenAI SDK にメモリ上の HTTP トランスポートを注入する。API キー不要。"""
...
def plot_data():
    return {
        ...
        "scenes": [{"name": "出会い", "opening": {"place": "家", "time_of_day": "朝", "weather": "晴れ"},
                    "beats": ["少女が家で魔人に出会う"], "covers": [1], "target_chars": 1000}],
    }
...
class M1Test(unittest.TestCase):
    def setUp(self):
        ...
        # 例は全フェーズ null。大半のテストは明示的な true/false の挙動を見るので上書きする。
        for name, phase in self.config_data["local"]["phases"].items():
            phase["thinking"] = name not in ("writing", "baseline")
        self.save_config()
    ...
    def test_explicit_thinking_requires_body_and_rejects_output_overrides(self):
        phase = self.config.phase("baseline")  # thinking=False
    ...
    def test_null_thinking_sends_nothing_and_needs_no_options(self):
        example = json.loads((PROJECT_ROOT / "config/backends.example.json").read_text(encoding="utf-8"))
        self.assertTrue(all(phase["thinking"] is None for phase in example["local"]["phases"].values()))
        phase = self.config.phase("baseline").model_copy(update={"thinking": None})
        self.assertEqual(thinking_body({}, phase), {})
    ...
    def test_baseline_writes_utf8_input_and_body_four_distinct_times(self):
        ...
        self.assertEqual(len(self.requests), 4)  # 暗黙の接続テスト呼び出しはしない
        self.assertFalse(self.requests[0]["test_thinking"])
        self.assertEqual(self.requests[0]["model"], self.config.phase("writing").model)

    def test_baseline_runs_with_null_thinking_and_no_options(self):
        for phase in self.config_data["local"]["phases"].values():
            phase["thinking"] = None
        self.save_config()
        factory = self.factory(lambda request: sdk_http.Response(200, json=completion()))
        output = baseline(self.input, config_path=self.config_path,
                          output_root=self.root / "outputs", gateway_factory=factory)
        self.assertEqual((output / "baseline.md").read_text(encoding="utf-8"), "小説本文。")
        self.assertNotIn("test_thinking", self.requests[0])
        log = self.logs(output / "calls.jsonl")[0]
        self.assertIsNone(log["thinking"])
        self.assertFalse(log["thinking_request_supplied"])
    ...
    def test_structured_response_uses_actual_spec_schema_and_validates(self):
        with self.gateway(lambda request: sdk_http.Response(200, json=completion(json.dumps(plot_data())))) as gateway:
            plot = gateway.parse(create_message(), Plot)
        self.assertEqual(plot.scenes[0].opening.place, "家")
        self.assertEqual(self.requests[0]["response_format"]["json_schema"]["name"], "Plot")
        ...
        with self.gateway(lambda request: sdk_http.Response(200, json=completion('{"Scenes": []}'))) as gateway:
            with self.assertRaises(ValidationError):
                gateway.parse(create_message(), Plot)
    ...
    def test_cli_help_and_import_do_not_load_search_or_secrets(self):
        original = builtins.__import__
        def guarded(name, *args, **kwargs):
            if name in ("util.tools", "util.settings", "tavily", "dotenv"):
                raise AssertionError("main.py must not import legacy secret/search modules")
