...
def model_snapshot(config):
    """LM Studio のネイティブ API でロード状態を読む。生成自体は gateway 経由のまま。"""
    import httpx
...
def memory_snapshot():
    """Windows のホスト RAM・LM Studio プロセス・GPU アダプタメモリを採取する。"""
    ...
    if "schema" in checks:
        ...
        for index in range(1, 11):
            ...
            item, response = invoke(label, phase,
                                    ...
                                    "constraints にはジャンルや属性を、events にはユーザ入力の出来事を入力の順序どおりに列挙してください。\n" + idea,
